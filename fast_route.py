...
class Fast_Route:
    ...
    def __init__(self):
        self.lectura_calles = np.loadtxt("data\Calles.txt", dtype=np.str_ , delimiter=",")
        self.letura_intersecciones = np.loadtxt("data\Interseccion.txt",  delimiter=",", converters= lambda x: float(x))
        self.calles = dict()
    ...
    def leer_archivos(self):
        ...
        for columna in self.letura_intersecciones:
            self.intersecciones[int(columna[0]), int(columna[1])] = interseccion.Interseccion(int(columna[0]), int(columna[1]), columna[2], columna[3])
    ...
    def realizar_grafo(self):
        ...
        for clave in self.intersecciones:
            self.grafo_ruta.agregarArista(int(self.intersecciones[clave].calle1Id), int(self.intersecciones[clave].calle2Id), random.randint(1, 100))
    
    def realizar_dijkstra(self, inicio):
        self.grafo_ruta.dijkstra(inicio)
    
    def hallar_camino_corto(self, inicio, final):
        print(self.grafo_ruta.camino(inicio, final))
